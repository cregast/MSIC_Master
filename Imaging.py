try:
    from PIL import Image, ImageDraw                                               ## PIL and Pillow should work interchangeably here but PIL is preffered
...
def ShowImage(medData, settings):
    ...
    numLines = len(medData)
    numIntensities = len(medData[0].intensities)
    w = numIntensities
    pixW = 1
    ...
    for smallData in medData:
        ...
        for intensity in smallData.intensities:
            try:
                color = CMaptoColor(CMAP(intensity / (medData[0].maxInt)))
            except ZeroDivisionError:
                color = CMaptoColor(CMAP(0))
            draw.rectangle(((i * pixW, (smallData.lineNum - 1) * pixH), (i * pixW + pixW, (smallData.lineNum - 1) * pixH + pixH)), fill = color)
            i += 1
...
def CreateRGB(bigData, save):                                         ## How to make sure that people only use 3 images from same exp.
    ...
    numLines = len(bigData[0])
    numIntensities = len(bigData[0][0].intensities)
    ...
    for smallData in bigData[2]:
        i = 0
        for intensity in smallData.intensities:
            color = (int(intensity * scale3), int(bigData[0][j].intensities[i] * scale1), int(bigData[1][j].intensities[i] * scale2))
            draw.rectangle(((i * pixW, (smallData.lineNum - 1) * pixH), (i * pixW + pixW, (smallData.lineNum - 1) * pixH + pixH)), fill = color)
            i += 1
        j += 1
    if save:
        output.save('MZ_%0.4f DT_%0.2f %s.png' % (float(bigData[0][0].mz), float(bigData[0][0].driftTime), bigData[0][1].name))
    else:
        ...
        for smallData in medData:
            ...
            for intensity in smallData.intensities:
                try:
                    ...
                    color = CMaptoColor(CMAP(0))
                draw.rectangle(((j * pixW, (smallData.lineNum - 1) * pixH), (j * pixW + pixW, (smallData.lineNum - 1) * pixH + pixH)), fill = color)
                j += 1

        if save:
            output.save(('MZ_%0.4f DT_%0.2f %s' + IMAGEFORMAT) % (float(medData[0].mz), float(medData[0].driftTime), medData[1].name))
        else:
            print('MZ_%0.4f DT_%0.2f' % (float(medData[0].mz), float(medData[0].driftTime)), end='')
            display(output)
        i += 1
