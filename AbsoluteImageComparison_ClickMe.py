...
if userInput == '1':
    ...
    while True:
        ...
        if (option == 1): # Add another image
            ...
            print('Didn\t recognize your selection. Please try again.')
    ...
    startTime = time.time()
    print('Scanning both directories for identical files')
    ...
    for filename in glob.glob('*.txt'):
        ...
        if filename in listOfFiles:
            medData = OpenTxt(folder1 + '/Images/' + filename)
            bigData.append(medData)
    ...
    pathlib.Path(settings[0] + '/Images/' + experiment1 + ' vs ' + experiment2).mkdir(parents=True, exist_ok=True)
    os.chdir(settings[0] + '/Images/' + experiment1 + ' vs ' + experiment2)

    i = 1
    AICMod1 = folder1.split('/')[-1]
    AICMod2 = folder2.split('/')[-1]
    for bigData in megaData:
        sys.stdout.write('\rProcessing image %d of %d' % ((i + len(megaData)), (len(megaData) * 2)))
