...
def RemoveSpikesLocal(medData):
    ...
    temp = medData
    boxSize = max(5, int(len(medData[0].intensities) / 10)) # Tweakable
    if boxSize % 2 == 0:
        boxSize += 1
        ...
        for intensity in smallData.intensities:
            try:
                ...
                while p <= offset:
                    ...
                    while q <= offset:
                        if (p != 0 or q != 0):
                            neighbours.append(medData[i + p].intensities[j + q])
                        q += 1
                    p += 1

                MU = mean(neighbours)
                STDEV = std(neighbours)
                upperBound = MU + (3 * STDEV)

                if (intensity > upperBound and intensity > 1000): # Doesn't affect pixels with intensity less than 1000, this can be changed as you see fit
                    neighbours = [medData[i-1].intensities[j-1],
                                  ...
                                  medData[i+1].intensities[j-1],
                                  medData[i  ].intensities[j-1]]
                    temp[i].intensities[j] = median(neighbours)
            except IndexError:
